import MeuPerfil from '@/components/promoter/ModalMeuPerfil';
...
test('renderiza corretamente o componente MeuPerfil e atualiza os campos corretamente', () => {
    ...
    expect(screen.getByLabelText('Nome:')).not.toHaveValue('Jane');
    expect(screen.getByLabelText('Sobrenome:')).not.toHaveValue('Smith');
    expect(screen.getByLabelText('E-mail:')).not.toHaveValue('jane.smith@example.com');
    expect(screen.getByLabelText('CPF:')).not.toHaveValue('98765432101');
});
